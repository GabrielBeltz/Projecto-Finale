...
public class EnemyCharger : MonoBehaviour
{
    ...
    public float HeightDistanceToSeePlayer = 2, WidthDistanceToSeePlayer = 7.5f;
    ...
    void Update()
    {
        bool playerHere = CheckPlayerPosition();
        if(playerHere) LookAtPlayer();
        
        _patrolScript.speed = playerHere ? originalSpeed * SeeingPlayerSpeedMultiplier  * Mathf.Sign(_patrolScript.speed) : originalSpeed * Mathf.Sign(_patrolScript.speed);
    }
    ...
    bool CheckPlayerPosition()
    {
        ...
        if(raycastHit2Ds.Length < 1) return false;
